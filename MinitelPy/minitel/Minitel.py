...
from multiprocessing import Process   # Processus pour l’émission/réception
...
class Minitel:
    ...
    Compte tenu de son fonctionnement en multi-process, le programme principal
    ...
    def __init__(self, peripherique = '/dev/ttyUSB0'):
        ...
        # Initialise un drapeau pour l’arrêt des processus
        self._continuer = True

        # Crée les deux processus de lecture/écriture
        self._processes = []
        self._processes.append(Process(target=self._gestion_entree))
        self._processes.append(Process(target=self._gestion_sortie))

        # Démarre les deux processus de lecture/écriture
    ...
    def close(self):
        ...
        Indique aux processus d’émission/réception qu’ils doivent s’arrêter et
        ...
        s’exécuter.
        """
        # Indique aux processus qu’ils doivent arrêter toute activité
        self._continuer = False

        # Attend que tous les processus aient fini
        for process in self._processes:
            process.join()
